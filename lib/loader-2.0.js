...
async function go(list, db, interval, range, startTime, checkExist, prefix, suffix) {
    ...
    for (let i = 0, len = list.length; i < len; i++) {
        ...
        let stock = list[i];
        let collectionName = prefix + stock.code + suffix;
        let info = [
            `name: ${collectionName}`,
            `total: ${list.length}`,
            `current: ${i}`,
        ...
        if (stock.isFailInYahoo || stock.volume < 100 * 10000 || stock.current < 10 || stock.current > 70 || stock.instOwn > 60) {
            console.log('jump');
            continue;
        }
        if (checkExist) {
